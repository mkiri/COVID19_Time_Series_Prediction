...
#creating spark dataframe from table of basic feature set

df = spark.table('g6_confirmed_cases_simple_csv')
...
print(smape(A_train, F_train))
 #looking at the values it is clear the model is overfitting on the training data, with low bias and high variance. This is supported by a high r2 score in train and a poor r2 in test. The SMAPE is satisfactory as 6.67% for test data.
...
#training data
# select line chart with canada_actual and prediction under values and date under key
# from this graph it can be seen that the data is slightly overfitting on the training data. 
display(pred_train)
...
print('SMAPE train:')
print(smape(A_train, F_train))
 #looking at the values it is clear the model is overfitting on the training data, with low bias and high variance. 
